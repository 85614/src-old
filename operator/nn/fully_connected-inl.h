...
namespace mxnet
{
  namespace op
  {
    ...
    namespace fullc
    {
      enum FullyConnectedOpInputs
      {
        kData,
        kWeight,
        kBias
      };
      enum FullyConnectedOpResource
      {
        kTempSpace
      };
      enum FullyConnectedOpOutputs
      {
        kOut
      };
    ...
    struct FullyConnectedParam : public dmlc::Parameter<FullyConnectedParam>
    {
      ...
      DMLC_DECLARE_PARAMETER(FullyConnectedParam)
      {
        ...
        DMLC_DECLARE_FIELD(num_hidden).set_lower_bound(1).describe("Number of hidden nodes of the output.");
        DMLC_DECLARE_FIELD(no_bias).set_default(false).describe("Whether to disable bias parameter.");
        DMLC_DECLARE_FIELD(flatten).set_default(true).describe("Whether to collapse all but the first axis of the input data tensor.");
      }
...
*/
                         ...
                         Tensor<cpu, 2, DType> out, Stream<cpu> *s)
    {
      ...
      cl_kernel kernel;
      auto kernelManger = manager.make_kernel(kernel, kernel_name, program_src);
